 ...
 * @author krygin
 */
public class AddNewInterviewerUserWindow extends Window implements FieldEvents.BlurListener{
    ...
    public AddNewInterviewerUserWindow(AdminUserManagement adminUserManagement) {
        this.adminUserManagement = adminUserManagement;
        setModal(true);
        setWidth("30%");
        setResizable(false);
        center();
        setCaption("New Interviewer User");
        ...
        layout.setComponentAlignment(okBut, Alignment.MIDDLE_LEFT);

        /*Button cancelBut = new Button("cancel");
        okBut.addListener(new Button.ClickListener() {
            public void buttonClick(Button.ClickEvent event) {
                AddNewInterviewerUserWindow.this.close();
            }
        });
        layout.addComponent(cancelBut);
        layout.setComponentAlignment(cancelBut, Alignment.MIDDLE_LEFT);*/
    }

    /*private void buttonClickCancel() {
        if (AddNewInterviewerUserWindow.this != null) {
            AddNewInterviewerUserWindow.this.close();
        }
    }*/
