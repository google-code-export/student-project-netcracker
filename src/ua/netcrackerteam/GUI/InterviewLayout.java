...
class InterviewLayout extends VerticalLayout {
		...
		save.addListener(new Button.ClickListener() {
			...
			public void buttonClick(Button.ClickEvent clickEvent) {
				if (daySelector.getOldTimeSelector().getValue() != null
						|| (selectedInterview != null && selectedInterview.equals(nullInterview))) {
					saveInterview();
				}
				else {
					getWindow().showNotification("Выберите время собеседования!",
	...
	private void showEditComponents() {
			...
			public void buttonClick(Button.ClickEvent clickEvent) {
				if (validTime() || selectedInterview.equals(nullInterview)) {
					...
					getWindow().showNotification(
							"Вы не можете перерегистрироваться за пол часа до начала выбранного собеседования.",
							Window.Notification.TYPE_TRAY_NOTIFICATION);
				}
			}
		});
		Button sendPDF = new Button("Отправить PDF");
		sendPDF.setDescription("Отправить анкету на e-mail адрес");
		...
		Button.ClickListener() {
			...
			public void buttonClick(Button.ClickEvent clickEvent) {
				...
				getWindow().showNotification("Благодарим за регистрацию! Анкета отправлена на Ваш e-mail адрес.",Window.Notification.TYPE_TRAY_NOTIFICATION);
			}
	...
	private void showLabels() {
		...
		verticalLayout.addComponent(caption);
		caption = new Label(
				"Внимание! Вы не сможете перерегистрироваться за пол часа до начала выбранного собеседования.");
		verticalLayout.addComponent(caption);
	}
	...
	private void loadRegistrationPanelSettings() {
		...
		try {
			...
			File file = new File(context.getHttpSession().getServletContext()
					.getRealPath("/WEB-INF/resources/interview-text.txt"));
	...
	private class DaySelector extends ComboBox {
		...
		public DaySelector() {
			...
			BeanItemContainer<StudentInterview> daysContainer = new BeanItemContainer(StudentInterview.class,
					interviewDays);
	...
	private class TimeSelector extends Table {
		...
		public TimeSelector(DaySelector daySelector) {
			...
			BeanItemContainer<StudentInterview> interviewContainer = new BeanItemContainer<StudentInterview>(
					StudentInterview.class, availableTimes);
			setWidth(400, UNITS_PIXELS);
			setHeight("50%");
			setSelectable(true);
			setImmediate(true);
